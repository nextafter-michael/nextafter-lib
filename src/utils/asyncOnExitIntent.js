/**
 * asyncOnExitIntent - Executes a function after a delay is completed and the exit intent conditions are triggered.
...
function asyncOnExitIntent (functionRef = () => {}, delay = 0, ...args) {
    return new Promise((resolve, reject) => {
        const onDocumentExit = (event) => { // define a function that will be called when elements are mouseout'd
            if (!event.toElement && !event.relatedTarget) { // only continue if the next element is null (outside of the document)        
                ...
                resolve(functionRef.call(null, ...args)); // run the code that is supposed to run on exit intent
...
export default asyncOnExitIntent;